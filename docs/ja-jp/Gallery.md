<div align="right" style="margin-bottom: 20px;">
  <details>
    <summary style="background: #2E3440; color: #D8DEE9; border: 1px solid #4C566A; border-radius: 6px; padding: 8px 16px; cursor: pointer; font-size: 14px; display: inline-flex; align-items: center; gap: 8px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif; list-style: none;">🌐 言語</summary>
    <div style="margin-top: 10px; padding: 12px; background: #3B4252; border: 1px solid #4C566A; border-radius: 6px; text-align: right;">
      <div style="margin-bottom: 6px;"><a href="Gallery" style="color: #88C0D0; text-decoration: none;">🇺🇸 English</a></div>
      <div style="margin-bottom: 6px;"><a href="Gallery-pt-br" style="color: #88C0D0; text-decoration: none;">🇧🇷 Português</a></div>
      <div style="margin-bottom: 6px;"><a href="Gallery-es-es" style="color: #88C0D0; text-decoration: none;">🇪🇸 Español</a></div>
      <div style="margin-bottom: 6px;"><a href="Gallery-fr-fr" style="color: #88C0D0; text-decoration: none;">🇫🇷 Français</a></div>
      <div style="margin-bottom: 6px;"><a href="Gallery-de-de" style="color: #88C0D0; text-decoration: none;">🇩🇪 Deutsch</a></div>
      <div style="margin-bottom: 6px;"><a href="Gallery-it-it" style="color: #88C0D0; text-decoration: none;">🇮🇹 Italiano</a></div>
      <div style="margin-bottom: 6px;"><a href="Gallery-ja-jp" style="color: #88C0D0; text-decoration: none;">🇯🇵 日本語</a></div>
      <div style="margin-bottom: 6px;"><a href="Gallery-zh-cn" style="color: #88C0D0; text-decoration: none;">🇨🇳 中文</a></div>
      <div><a href="Gallery-ru-ru" style="color: #88C0D0; text-decoration: none;">🇷🇺 Русский</a></div>
    </div>
  </details>
</div>

# ギャラリー

Hentai-Senpai GTK テーマのビジュアルショーケース。

## テーマプレビュー

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>アプリケーション</h3>
      <img src="https://raw.githubusercontent.com/PhantomNimbi/Hentai-Senpai-GTK-Theme/main/images/preview-1.png" alt="アプリケーションプレビュー" width="100%"/>
      <p>ボタン、メニュー、ウィンドウ装飾の細部にまでこだわった、一貫したスタイルのGTKアプリケーション。</p>
    </td>
    <td width="50%" valign="top">
      <h3>デスクトップ</h3>
      <img src="https://raw.githubusercontent.com/PhantomNimbi/Hentai-Senpai-GTK-Theme/main/images/preview-2.png" alt="デスクトッププレビュー" width="100%"/>
      <p>クリーンでエレガントな外観のためのNordカラーパレットを備えた美しいダークテーマを紹介する、完全なデスクトップ体験。</p>
    </td>
  </tr>
</table>

## 壁紙

テーマには、モノクロカラーとエッチなテーマを特徴とするミニマルな壁紙コレクションが含まれています。

<table>
  <tr>
    <td width="33%" valign="top" style="padding: 10px;">
      <img src="https://raw.githubusercontent.com/PhantomNimbi/Hentai-Senpai-GTK-Theme/main/wallpapers/001.png" alt="壁紙 001" width="100%" style="border-radius: 8px;"/>
      <p align="center"><strong>001</strong></p>
    </td>
    <td width="33%" valign="top" style="padding: 10px;">
      <img src="https://raw.githubusercontent.com/PhantomNimbi/Hentai-Senpai-GTK-Theme/main/wallpapers/002.png" alt="壁紙 002" width="100%" style="border-radius: 8px;"/>
      <p align="center"><strong>002</strong></p>
    </td>
    <td width="33%" valign="top" style="padding: 10px;">
      <img src="https://raw.githubusercontent.com/PhantomNimbi/Hentai-Senpai-GTK-Theme/main/wallpapers/003.png" alt="壁紙 003" width="100%" style="border-radius: 8px;"/>
      <p align="center"><strong>003</strong></p>
    </td>
  </tr>
  <tr>
    <td width="33%" valign="top" style="padding: 10px;">
      <img src="https://raw.githubusercontent.com/PhantomNimbi/Hentai-Senpai-GTK-Theme/main/wallpapers/004.png" alt="壁紙 004" width="100%" style="border-radius: 8px;"/>
      <p align="center"><strong>004</strong></p>
    </td>
    <td width="33%" valign="top" style="padding: 10px;">
      <img src="https://raw.githubusercontent.com/PhantomNimbi/Hentai-Senpai-GTK-Theme/main/wallpapers/005.png" alt="壁紙 005" width="100%" style="border-radius: 8px;"/>
      <p align="center"><strong>005</strong></p>
    </td>
    <td width="33%" valign="top" style="padding: 10px;">
      <img src="https://raw.githubusercontent.com/PhantomNimbi/Hentai-Senpai-GTK-Theme/main/wallpapers/006.png" alt="壁紙 006" width="100%" style="border-radius: 8px;"/>
      <p align="center"><strong>006</strong></p>
    </td>
  </tr>
</table>

## 壁紙のダウンロード

すべての壁紙はリポジトリの `wallpapers/` ディレクトリで利用できます。個別にダウンロードするか、リポジトリ全体をクローンしてすべてを取得できます。

```bash
# すべての壁紙を取得するためにリポジトリをクローン
git clone https://github.com/PhantomNimbi/Hentai-Senpai-GTK-Theme.git
cd Hentai-Senpai-GTK-Theme/wallpapers
```

## カラーパレット

このテーマは [Nord カラーパレット](https://www.nordtheme.com/)を使用しています：

- **Polar Night** - 暗い背景色
- **Snow Storm** - 明るいテキスト色
- **Frost** - 青いアクセント色
- **Aurora** - カラフルなアクセント色

詳細なカラー情報については、[カラーパレット](Color-Palette-ja-jp) ページをご覧ください。
